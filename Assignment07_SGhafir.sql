...
-- 2017-01-01,SGhafir,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_SGhafir')
	 Begin 
	  Alter Database [Assignment07DB_SGhafir] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_SGhafir;
	 End
	Create Database Assignment07DB_SGhafir;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_SGhafir;
...
SELECT 
    ...
    FORMAT(Products.UnitPrice, 'C', 'en-US') AS PriceInUSD
FROM 
    Categories
JOIN 
...
FROM 
    Products
JOIN 
    Inventories ON Products.ProductID = Inventories.ProductID
ORDER BY 
    Products.ProductName, 
...
Select * From [dbo].[vProductInventories] order by 1,2;
go
...
Select 
	...
	InventoryCount,
	PreviousMonthCount,
	[previousmonthanalysis] = isnull(case
...
as 
	return select
		...
		where previousmonthanalysis = @KPI;
